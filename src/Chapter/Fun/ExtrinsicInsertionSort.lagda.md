...
lower-bound-insert {x} {y} {z ∷ ys} x≤y (lb-∷ x≤z x≤ys) with ≤-total y z
... | inj₁ y≤z = lb-∷ x≤y (lb-∷ x≤z x≤ys)
... | inj₂ z≤y = lb-∷ x≤z (lower-bound-insert x≤y x≤ys)
```
...
sorted-insert x (y ∷ ys) (sorted-∷ y≤ys ys-sorted) with ≤-total x y
... | inj₁ x≤y = sorted-∷ (lb-∷ x≤y (lower-lower-bound x≤y y≤ys))
                          (sorted-∷ y≤ys ys-sorted)
... | inj₂ y≤x = sorted-∷ (lower-bound-insert y≤x y≤ys)
...
insertion-sort-permutation (x ∷ xs) = #begin
