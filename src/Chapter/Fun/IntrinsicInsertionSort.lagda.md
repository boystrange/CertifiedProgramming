...
```
intrinsic-insert x [] sorted-[] = [ x ] , #refl , singleton-sorted x
```
...
```
intrinsic-insert x (y ∷ ys) (sorted-∷ y≤ys ys-sorted) with ≤-total x y
```
...
```
... | inj₂ y≤x with intrinsic-insert x ys ys-sorted
```
